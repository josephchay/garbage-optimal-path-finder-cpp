...
public: //Public specifier
    ...
    void tspPath(int parentLocation[], int index, const vector<string>& currentLocations, ofstream& outFileTSP, stringstream& buffer) { //Find the path for the TSP algorithm
        ...
        tspPath(parentLocation, parentLocation[index], currentLocations, outFileTSP, buffer); //Recursively call the function to find the path, this is so that the path is printed in the correct order
    ...
    metrics::Metrics tspSolution(float distance[], int vertex, int parent[], const vector<string>& currentLocations, const GarbageLocations& garbageLocations, ofstream& outFileTSP, bool searchForBest) { //Implement the TSP solution
        stringstream buffer; //Create a stringstream buffer
        ...
        for (int index = 1; index < VERTICES; index++) { //Iterate over the number of vertices which is the number of locations
            if (garbageLocations.wasteLevels.at(currentLocations[index]) >= 60) { //If the waste level is greater than or equal to 60
                ...
                if (distance[index] <= 12) { //If the distance is less than or equal to 12
                    ...
                    tspPath(parent, index, currentLocations, outFileTSP, buffer); //Call the tspPath method to find the path
...
//                        buffer << "**Total time exceeds 8 hours, refer to the locations below that can be visited within 8 hours**" << endl;
                        break;
                    }
                } else { //If the distance exceeds 12 km
                    buffer << "Note: Location not visited due to distance exceeding 12km";
        ...
        results.name = "Travelling Salesman Problem Route";
        ...
        if (!searchForBest) { //If the search for best is false
            cout << buffer.str() << endl; //Then output the buffer
        }

        return results;
    }

    metrics::Metrics travellingSalesmanProblem(int garbageLocationGraph[][VERTICES], const vector<string>& currentLocations, const GarbageLocations& garbageLocations, ofstream& outFileTSP, bool searchForBest) { //Implement the travelling salesman problem algorithm
        ...
        auto results = tspSolution(dist, VERTICES, parent, currentLocations, garbageLocations, outFileTSP, searchForBest); //Call the tspSolution function
