...
//        outFile << std::setw(1) << " "; //Print the distance matrix and include formatting using std::setw
//        std::cout << std::setw(1) << " "; //Print the distance matrix and include formatting using std::setw
//        for (std::vector<std::string>::const_iterator iterator = locations.begin(); iterator != locations.end(); iterator++) { //Print the distance matrix and include formatting using std::setw
//            outFile << std::setw(1) << *iterator << " ";
//            std::cout << std::setw(1) << *iterator << " ";
//        }

        outFile << std::endl;
        std::cout << std::endl;

        for (int index=0; index < locations.size(); index++) { //Print the distance matrix and include formatting using std::setw
            outFile << std::setw(1) << locations[index] << ": ";
            std::cout << std::setw(1) << locations[index] << ": ";
            for (int index2 = 0; index2 < locations.size(); index2++) {  //iterate over the locations
                if (distanceMatrix[index][index2] != INF) { //If there is a direct path because it is != INF then print the distance and display
                    outFile << std::setw(1) << distanceMatrix[index][index2] << " ";
                    std::cout << std::setw(1) << distanceMatrix[index][index2] << " ";
                } else { //If there is no direct path because it is == INF then print INF and display
                    outFile << std::setw(1) << "INF "; // Print INF values to indicate no direct path
                    std::cout << std::setw(1) << "INF "; // Print INF values to indicate no direct path
                }
            }
